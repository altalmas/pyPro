...
while True:
    ...
    # now, to filter out alot of unnecessary contours, we sort the controus from the 
    # biggest to the smallest, and then we care about the biggest only
    contours = sorted(contours, key=lambda x:cv2.contourArea(x), reverse=True)

    # if there is more than one object to capture, we filter out the contours that are unimportant
    for cnt in contours:
        ...
        if area >= 50:
            ...
            cv2.line(frame, (0,int(y+(h/2))), (dispW,int(y+(h/2))), (255,0,0), 3 )
            cv2.line(frame, (int(x+(w/2)),0), (int(x+(w/2)),dispH), (255,0,0), 3 )


    #cv2.drawContours(frame, contours, 0, (255,0,0), 3) # -1 means draw all contours
